...
public class RoomTemplateScriptableObject : ScriptableObject
{
    ...
    #region Tooltip
    [Tooltip("The room node type Scritpable Object. The room node types correspond to the room nodes used in the room node graph.  The exceptions being with corridors.  In the room node graph there is just one corridor type 'Corridor'.  For the room templates there are 2 corridor node types - CorridorNS and CorridorEW.")]
    #endregion Tooltip
    ...
    #region Tooltip
    [Tooltip("Populate the list with all teh enemies that can be spawned in this room by dungeon level, including the ration (random) of this enemy type that will be spawned")]
    #endregion Tooltip
    public List<SpawnableObjectsByLevel<EnemyDetailsScriptableObject>> enemiesByLevelList;
    ...
    private void OnValidate()
    {
        ...
        if (enemiesByLevelList.Count > 0 || roomEnemySpawnParametersList.Count > 0)
        {
            ...
            foreach (RoomEnemySpawnParameters roomEnemySpawnParameters in roomEnemySpawnParametersList)
            {
                ...
                foreach (SpawnableObjectsByLevel<EnemyDetailsScriptableObject> dungeonObjectsByLevel in enemiesByLevelList)
                {
                    ...
                    foreach (SpawnableObjectRatio<EnemyDetailsScriptableObject> dungeonObjectRatio in dungeonObjectsByLevel.spawnableObjectRatioList)
                    {
